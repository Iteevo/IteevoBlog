...
### 1.算法(algorithms)是一步步地解决AI问题的过程，比如生成移动到目标的路径，决定要移动的方向已拦截逃跑的敌人，决策角色下一步要做的动作等等。数据结构是算法的另一面，数据结构为我们的算法保存提供数据，而数据与游戏世界的表现息息相关。
...
![](/img/in-post/Introduction_to_game_AI/PUSH-link-rel-preload.jpg)
